...
| **Latent variables & SEM — slice B** — CB-SEM (lavaan), PLS-SEM (seminr), path analysis (observed-only), embedded mediation live → **catalog 48/48**; AMOS-style `SemCanvas` (draw/move/delete/zoom/pan/resize + post-run β/loading/R² overlay + `captureNode` figure export); seminr bootstrap runs in WebR via the makeCluster serial shim; WebR ≡ native R verified (PoliticalDemocracy, mobi); §6 spec-card amendments; catalog/docs/per-test-docs (46/47) updated; **2 prod bugs found+fixed via real-app runs** (results-screen figure-mount; matrix-domId export). Final gate GREEN @ `7b58ad2` (tsc · build · test:fast ×2 · WebR vitest 1309/1309 · e2e 18/18 · fresh-clone). **Follow-up in progress: path-mode canvas UI bridge** (path-analysis runnable from the canvas; engine/export already native-R-proven). | 2026-06-21 | `specs+plans/2026-06-20-telos-sem-cbsem-plssem-canvas*`, ratify `reviews/2026-06-21-sem-b-build-final-ratify.md` |

## Remaining (in order)

| # | Slice | Tests / scope | Est. sessions | Risk notes |
|---|---|---|---|---|
| 1 | Polish & launch | design pass, DRAFT copy confirmations, B-list, feedback URL, analytics, licence, a11y, deploy | ~1 | mostly owner decisions. **Carries the SEM-B follow-up: path-mode canvas UI bridge** (wire `modelKind:'path'` + seed nodes from columns so path-analysis runs from the canvas). |

> **Agenda (no date yet, owner-flagged 2026-06-21): Visual redesign — "make the app look good."** A production-grade visual/frontend polish pass — more substantial than slice 2's lighter "design pass." Use the `frontend-design` skill; either preserve the locked design language (white-dominant, blue `#185fa5`, Crimson Pro display / Atkinson Hyperlegible UI, warm paper `#f0efe9`) or evolve it per owner direction. Owner picks the slot.

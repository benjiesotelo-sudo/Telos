...
- **D3 — Architecture: replicate the format in the TS builders** (preserves theming, consistency tests, figures, report-only neutralization). Do **not** render tables by running modelsummary live. *(The "emit a real `modelsummary()` call in the exported R script" half is **DEFERRED** — the R-script export doesn't exist yet [3 export formats are disabled "later slice"]; building it is a separate future slice. The canonical call is captured in the spike doc §3, ready for that slice. This slice = the in-app restyle. Ratify flag.)*

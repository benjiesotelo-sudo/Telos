...
describe('buildExportFiles (Task 10)', () => {
  ...
  // construct-slots form is hidden - nodes/paths come from setup.placed, P2 shelf model). The runner
  // already seeds those constructs; the export seam must too, or analysis.R is the empty CB-SEM stub.
  it('path-analysis: seeds path-mode constructs from placed columns so analysis.R is REAL lavaan, not the stub', () => {
    ...
    const s = {
      ...
      // stored constructs are EMPTY in path mode; nodes/paths come from setup.placed by index.
    ...
    // structural regressions among the placed columns (path.to ~ path.from): x2 ~ x1, x3 ~ x2

...
function Slot({ spec, role, assigned, onRemove, onHoverChange }: {
  spec: TestSpec; role: RoleConstraint; assigned: string[]; onRemove: (roleId: string, column: string) => void
  onHoverChange: (roleId: string | null) => void
}) {
...
export function DragSlotsUI({ spec, shelves, roles, onDrop, onRemove, echoRole }: {
  spec: TestSpec
  shelves: Shelf[]
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1.2 }}>
          {spec.constraints.roles.map((r) => (
            <Slot key={r.roleId} spec={spec} role={r} assigned={roles[r.roleId] ?? []} onRemove={onRemove} onHoverChange={setEcho} />
          ))}
        </div>
      </div>
    </DndContext>

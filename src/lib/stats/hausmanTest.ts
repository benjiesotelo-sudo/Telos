...
export interface HausmanCompareRow { term: string; feB: number; reB: number; diff: number }
...
compare_rows <- lapply(common, function(t) list(term = t, feB = unname(fe_c[t]), reB = unname(re_c[t]), diff = unname(fe_c[t] - re_c[t])))

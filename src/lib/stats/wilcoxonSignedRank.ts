...
# conf.int=TRUE adds the Hodges–Lehmann median-difference estimate + CI; suppressWarnings absorbs the benign
# "requested conf.level not achievable" R throws at small N (spike: n=6 paired) — pattern already used elsewhere.
wt <- if (force_approx) suppressWarnings(wilcox.test(a, b, paired=TRUE, exact=FALSE, correct=continuity, alternative=alternative, conf.int=TRUE))
      else suppressWarnings(wilcox.test(a, b, paired=TRUE, correct=continuity, alternative=alternative, conf.int=TRUE))
zt <- coin::wilcoxsign_test(a ~ b)
rb <- effectsize::rank_biserial(a, b, paired=TRUE, ci=level)
naNull <- function(x) if (length(x) && is.finite(x)) x else NULL   # guard NA/±Inf → null for the em-dash fx
list(ranks=list(srow('Positive', which(sg > 0)), srow('Negative', which(sg < 0)), srow('Ties', which(sg == 0))),
  ...
  hl=naNull(unname(wt$estimate)), hlLow=naNull(wt$conf.int[1]), hlHigh=naNull(wt$conf.int[2]))`
...
export interface WilcoxonSignedRankResult {
  ...
  hl: number | null; hlLow: number | null; hlHigh: number | null  // Hodges–Lehmann median-difference estimate + 95% CI (wilcox.test conf.int=TRUE); null when R cannot compute it
  alpha: number
  tails: string
  nExcluded: number
  figurePng: Uint8Array<ArrayBuffer>
}
interface RawStats { ranks: SignedRankRow[]; v: number; z: number; p: number; r: number; rLow: number; rHigh: number; method: string; hl: number | null; hlLow: number | null; hlHigh: number | null }
...
export async function runWilcoxonSignedRank(
  ...
  return { ranks: [s.ranks[0], s.ranks[1], s.ranks[2]], v: s.v, z: s.z, p: s.p, r: s.r, rLow: s.rLow, rHigh: s.rHigh, method: s.method, hl: s.hl ?? null, hlLow: s.hlLow ?? null, hlHigh: s.hlHigh ?? null, alpha, tails: alternative, nExcluded, figurePng }
}

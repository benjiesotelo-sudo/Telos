...
export interface IvCoefRow {
  term: string; b: number; se: number; t: number; p: number; ciLow: number; ciHigh: number
  olsB: number; olsSe: number; olsCiLow: number; olsCiHigh: number
}
export interface IvResult {
  firstStage: IvFirstStageRow[]
  coefRows: IvCoefRow[]
  weakF: number; weakP: number; wuF: number; wuP: number; sargan: number | null; sarganP: number | null
  structF: number; rmse: number          // structural Wald F = summary(.,diagnostics=T)$waldtest[1]; RMSE = sqrt(mean(resid²))
  endogenous: string[]
...
ols <- lm(as.formula(paste('.y ~', rhs)), data = d)            # OLS|2SLS shape — same RHS terms, names align by row
V  <- if (se_robust) sandwich::vcovHC(iv,  type = 'HC1') else stats::vcov(iv)
Vo <- if (se_robust) sandwich::vcovHC(ols, type = 'HC1') else stats::vcov(ols)
ct <- lmtest::coeftest(iv, vcov. = V); ci <- lmtest::coefci(iv, vcov. = V, level = ci_level)
oct <- lmtest::coeftest(ols, vcov. = Vo); oci <- lmtest::coefci(ols, vcov. = Vo, level = ci_level)
labs <- rownames(ct)
...
list(first_stage = first_stage, coef_rows = coef_rows,
  ...
  sargan = gd('Sargan', 'statistic'), sargan_p = gd('Sargan', 'p-value'),
  struct_f = unname(sm$waldtest[1]), rmse = sqrt(mean(residuals(iv)^2)), n_obs = nrow(d))`
...
interface RawIv {
  ...
  weak_f: number; weak_p: number; wu_f: number; wu_p: number; sargan: number | null; sargan_p: number | null
  struct_f: number; rmse: number; n_obs: number
}
...
export async function runIvTwoStage(
  ...
  return {
    ...
    structF: raw.struct_f, rmse: raw.rmse,

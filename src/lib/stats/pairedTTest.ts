...
export interface PairedTTestResult {
  ...
  r: number; rP: number                 // Pearson correlation between the two conditions (cor.test) — conventional alongside a paired t-test
...
list(stats = list(list(n = desc$n[1], mean = desc$mean[1], sd = desc$sd[1]),
  ...
  r = unname(ct$estimate), rP = ct$p.value,
...
interface RawStats { stats: { n: number; mean: number; sd: number }[]; t: number; df: number; p: number; meanDiff: number; ci: number[]; dz: number; dzLow: number; dzHigh: number; r: number; rP: number; shapiro: { W: number | null; p: number | null } }
...
export async function runPairedTTest(engine: Engine, data: Dataset, conditionA: string, conditionB: string, level = 0.95, alpha = 0.05, alternative = 'two.sided'): Promise<PairedTTestResult> {
  ...
  const env = { a: rows.map((r) => r[conditionA] as number), b: rows.map((r) => r[conditionB] as number), la: conditionA, lb: conditionB, level, alternative }
  ...
  return {
    ...
    dzLow: s.dzLow, dzHigh: s.dzHigh, r: s.r, rP: s.rP,

...
export const regressionEmitters: Record<string, Emitter> = {
  ...
  'granger-causality': (_spec, setup) => {
    ...
    return [
      ...
      `safe_max <- max(1L, min(${maxLag}L, floor((nrow(dd_ord) - 1L) / 2L)))`,
      `print(vars::VARselect(data.frame(x = dd_ord$${x}, y = dd_ord$${y}), lag.max = safe_max, type = "const")$criteria)`,
  ...
  'fixed-effects': (_spec, setup) => {
    ...
    return [
      ...
      `xb <- as.numeric(as.matrix(d[, c(${cols(regs)}), drop = FALSE]) %*% coef(fit)[c(${cols(regs)})])`,
      `agg <- aggregate(cbind(yy, xbxb) ~ idid, data.frame(idid = d$${entity}, yy = d$${y}, xbxb = xb), mean)`,
      `cat("Between R2:", cor(agg$yy, agg$xbxb)^2, "\\n")`,
      `cat("Overall R2:", cor(d$${y}, xb)^2, "\\n")`,
  ...
  'random-effects': (_spec, setup) => {
    ...
    return [
      ...
      `b_re <- coef(fit)`,
      `xb <- b_re[["(Intercept)"]] + as.numeric(as.matrix(d[, c(${cols(regs)}), drop = FALSE]) %*% b_re[c(${cols(regs)})])`,
      `id2 <- d$${entity}`,
      `dm2 <- function(v) ave(v, id2, FUN = function(x) x - mean(x))`,
      `cat("Within R2:", cor(dm2(d$${y}), dm2(xb))^2, "\\n")`,
      `agg <- aggregate(cbind(yy, xbxb) ~ idid, data.frame(idid = id2, yy = d$${y}, xbxb = xb), mean)`,
      `cat("Between R2:", cor(agg$yy, agg$xbxb)^2, "\\n")`,
      `cat("Overall R2:", cor(d$${y}, xb)^2, "\\n")`,
  ...
  'did': (_spec, setup, ds) => {
    ...
    return [
      ...
      `print(aggregate(${y} ~ tr + po, data = d, FUN = mean))`,
  ...
  'rdd': (_spec, setup) => {
    ...
    return [
      ...
      `h0 <- rd$bws[1, 1]`,
      `print(summary(rdrobust::rdrobust(d$${y}, d$${run}, c = ${cutoff}, p = ${poly}, level = 95, h = h0 / 2)))`,
      `print(summary(rdrobust::rdrobust(d$${y}, d$${run}, c = ${cutoff}, p = ${poly}, level = 95, h = h0 * 2)))`,
  ...
  'iv-2sls': (_spec, setup) => {
    ...
    return [
      ...
      ...endo.map((en) => `cat("\\n== First stage:", ${q(en)}, "==\\n"); print(summary(lm(${en} ~ ${ivpart}, data = d))$coefficients)`),
...
export const regressionPackages: Record<string, string[]> = {
  ...
  'granger-causality': ['lmtest', 'vars', 'ggplot2'],
  'var': ['vars'],

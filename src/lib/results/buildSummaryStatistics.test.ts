...
const overall: SummaryStatsResult = {
  grouped: false,
  rows: [
    { variable: 'score', n: 12, mean: 76.33333, sd: 7.08819, min: 66, max: 88, median: 76.5, skew: 0.11440, kurtosis: -1.49220, ciLow: 71.82973, ciHigh: 80.83694 },
    { variable: 'age', n: 1, mean: 34, sd: null, min: 34, max: 34, median: 34, skew: null, kurtosis: null, ciLow: null, ciHigh: null },
  ],
...
const byGroup: SummaryStatsResult = {
  grouped: true,
  rows: [
    { variable: 'score', group: 'control', n: 6, mean: 70.33333, sd: 3.14113, min: 66, max: 75, median: 70.5, skew: 0.06692, kurtosis: -1.52006, ciLow: 67.03690, ciHigh: 73.62977 },
    { variable: 'score', group: 'treatment', n: 6, mean: 82.33333, sd: 3.77712, min: 78, max: 88, median: 82, skew: 0.24881, kurtosis: -1.72169, ciLow: 78.36953, ciHigh: 86.29714 },
  ],
...
describe('buildSummaryStatistics', () => {
  it('plain branch: card columns untouched, one formatted row per variable, nulls as em-dashes', () => {
    ...
    expect(c.tables[0].spec.columns.map((col) => col.label)).toEqual(['Variable', 'N', 'M', 'SD', '95% CI', 'Min', 'Max', 'Median', 'Skew', 'Kurtosis (excess)'])
    expect(c.tables[0].spec.captionStyle).toBe('bare')
    expect(c.tables[0].rows[0]).toEqual({ variable: 'score', n: 12, mean: '76.33', sd: '7.09', ci: '[71.83, 80.84]', min: '66.00', max: '88.00', median: '76.50', skew: '0.11', kurtosis: '−1.49' })
  ...
  it('group-by branch: a Group column is INSERTED first and stats repeat per group — the registry spec object stays unmutated', () => {
    ...
    expect(c.tables[0].rows[1]).toEqual({ group: 'treatment', variable: 'score', n: 6, mean: '82.33', sd: '3.78', ci: '[78.37, 86.30]', min: '78.00', max: '88.00', median: '82.00', skew: '0.25', kurtosis: '−1.72' })
  ...
  it('carries the plain card note, the fixed APA sentence, and per-variable typed figures', () => {
    const c = buildSummaryStatistics(spec, overall)
    expect(c.note).toEqual({ kind: 'plain', text: 'one row per chosen variable; a Group column is added when "Group by" is used (stats repeat per group). 95% CI is the t-based confidence interval for the mean; kurtosis is excess (normal = 0).' })

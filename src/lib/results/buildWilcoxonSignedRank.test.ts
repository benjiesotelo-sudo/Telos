...
describe('buildWilcoxonSignedRank', () => {
  ...
  it('Table 2: one row under the literal V / W header, U+2212 minuses; r carries its 95% CI', () => {
    expect(c.tables[1].rows).toEqual([{ v: '0.00', z: '−2.20', p: '.031', r: '−1.00 [−1.00, −1.00]' }])
  })
  ...
  it('APA omits V/W — only Z, p, r with its CI, exactly as drawn', () => {
    expect(c.apa).toBe('A Wilcoxon signed-rank test gave Z=−2.20, p = .031, r=−1.00 [−1.00, −1.00].')
  })
  ...
  it('p-clause branch: tiny p renders p<.001 in table and sentence; midrank V keeps 2 dp; non-degenerate r CI renders both bounds', () => {
    ...
    expect(c2.apa).toContain('Z=−2.20, p < .001, r=−.86 [−.97, −.37].')
  })
})

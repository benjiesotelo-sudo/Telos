...
// logLik −29.379162, σ²=0.206076, Ljung–Box p=0.332572, n=48.
const res: ArimaSarimaResult = {
  ...
  aic: 64.758325, bic: 70.371928, loglik: -29.379162, sigma2: 0.206076, ljungboxP: 0.332572,
  forecastRows: [
...
describe('buildArimaSarima', () => {
  it('one coef table: stacked estimate/(SE)/[CI] per ARIMA term, a rule, then the 6 GOF rows', () => {
    ...
    expect(c.tables[0].rows).toEqual([
      ...
      { _kind: 'gof', term: 'Ljung–Box p', est: '.333' },
  ...
  it('keeps the plain table note, two figures, how-to-read; APA fills (p,d,q)/(P,D,Q)/AIC/Ljung–Box', () => {
    ...
    expect(c.apa).toBe('An ARIMA(1,0,0)(0,0,0) model was fit (AIC=64.76); the Ljung–Box test of residual autocorrelation gave p = .333.')
  })
})

...
const r: OneSampleTTestResult = {
  ...
  cohensDLow: 1.143717, cohensDHigh: 5.368138, // effectsize::cohens_d(ci=0.95) — native R ≡ WebR
  shapiro: { W: 0.96354, p: 0.84654 }, ciLevel: 0.95, alpha: 0.05, tails: 'two.sided', nExcluded: 2,
...
describe('buildOneSampleTTest', () => {
  ...
  it('branches: Shapiro null → em-dashes; p ≥ .001 stays a p= clause; negatives typeset U+2212', () => {
    ...
    expect(v.note!.text).toBe(`${spec.tableNote!.text} (Shapiro-Wilk W=—, p=—)`) // p null → no verdict appended
    expect(v.tables[1].rows[0]).toEqual({ mu0: '90', t: '−4.97', df: '5', p: '.004', mdiff: '−7.67', ci: '[−11.63, −3.70]', d: '−2.03 [−3.46, −0.55]' })
    expect(v.apa).toBe('A one-sample t-test gave M=82.3 vs. 90, t(5)=−4.97, p = .004, d=−2.03 [−3.46, −0.55].')
  })
})

...
export interface TableSpec { id: string; title: string; columns: ColumnDef[]; captionStyle?: 'bare' | 'preamble'; preambleLabel?: string; domId?: string; kind?: 'coef'; models?: ModelCol[]; gof?: GofRow[]; extraCols?: ColumnDef[] } // 'bare' renders "Table." (Summary, Distribution); 'preamble' renders "Table {preambleLabel}." (e.g. E1/E2 for an EFA stage ahead of a card's canonical Table 1-N — U3-T4) and is excluded from the canonical running count; default = numbered "Table N." · domId: DOM/capture id when the card-faithful id would collide on a combined results page (zip names keep id) · kind:'coef' = modelsummary stacked layout (see above)

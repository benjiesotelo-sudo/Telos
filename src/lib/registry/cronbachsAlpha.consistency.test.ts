...
import { CRONBACHS_ALPHA as spec } from './cronbachsAlpha'
...
describe("cronbachs-alpha registry stays faithful to the spec HTML (verbatim, card-scoped)", () => {
  ...
  it('numbered table captions equal the card captions (no captionStyle:bare on multi-table card)', () => {
    const caps = [...card.matchAll(/<div class="apa-cap"><b>Table \d+\.<\/b> (.*?)<\/div>/g)].map((m) => m[1])
    expect(caps).toEqual(spec.tables.map((t) => t.title))
    expect(spec.tables.every((t) => !t.captionStyle)).toBe(true)
  })
  ...
  it('options equal the inputs card option pills', () => {

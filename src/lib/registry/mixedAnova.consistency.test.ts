...
import { MIXED_ANOVA as spec } from './mixedAnova'
...
describe('mixed-anova registry stays faithful to the spec HTML (verbatim, card-scoped)', () => {
  it('table theads equal the card column sequences (sub tags stripped on both sides for entity-safe comparison)', () => {
    // strip() decodes entities AND removes tags; apply to both card th and spec composite so comparison is symmetric.
    ...
    expect(theads).toEqual(spec.tables.map((t) =>
      t.columns.map((c) => strip(c.sub ? `${c.label}<sub>${c.sub}</sub>` : c.label))))
  })

  it('numbered table captions equal the card captions (all four numbered)', () => {
  ...
  it('question, assume note, figure caption + type, how-to-read and R map match verbatim', () => {
  ...
  it('role sl-hint lines equal spec.roles hints', () => {

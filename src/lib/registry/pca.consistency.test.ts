...
const inCard = inputsHtml.slice(
  ...
  inputsHtml.indexOf('<div class="ttl">PLS-SEM</div>'),
)

describe('pca registry stays faithful to the spec HTML (verbatim, card-scoped)', () => {
  it('T1/T2 theads equal the card column sequences', () => {
    const theads = [...card.matchAll(/<thead>(.*?)<\/thead>/gs)].map((m) =>
      [...m[1].matchAll(/<th>(.*?)<\/th>/g)].map((t) => strip(t[1])),
    )
    // Both T1 and T2 are classic tables (columns.length > 0)
  ...
  it('T1/T2 numbered captions match spec', () => {
  ...
  it('no communality column appears in the T2 thead', () => {
    // PCA is data-reduction — must NOT surface a communality column
    const theads = [...card.matchAll(/<thead>(.*?)<\/thead>/gs)].map((m) =>
      [...m[1].matchAll(/<th>(.*?)<\/th>/g)].map((t) => strip(t[1])),
    )
    const t2Head = theads[1] ?? []
    expect(t2Head.every((h) => !h.toLowerCase().includes('communalit'))).toBe(true)
  })
})

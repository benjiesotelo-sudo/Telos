...
import { PROPENSITY_SCORE_MATCHING as spec } from './propensityScoreMatching'
...
const card = sliceTo(outputsHtml, '<span class="rt-name">Propensity score matching</span>', '<span class="rt-name">')
const inputsHtml = readFileSync('telos_test_inputs.html', 'utf8')
const inCard = sliceTo(inputsHtml, '<div class="ttl">Propensity score matching</div>', '<div class="ttl">')

describe('propensity-score-matching registry stays faithful to the spec HTML (verbatim, card-scoped)', () => {
  ...
  it('the ATT coef GOF footer stub labels equal spec.tables[1].gof labels in order (matched N — no R²/AIC)', () => {
  ...
  it('options equal the inputs card option strip (no α, no CI pill drawn — CI fixed at 95%)', () => {
    ...
    expect(spec.options.map((o) => o.kind)).toEqual(['display', 'number', 'select'])
  })
  it('PSM backbone: outcome interval/ratio, treatment nominal 2-cat, covariates numeric v1', () => {
    const c = (id: string) => spec.constraints.roles.find((r) => r.roleId === id)
    expect(c('outcome')!.levels).toEqual(['interval', 'ratio'])
    expect(c('treatment')!.levels).toEqual(['nominal', 'ordinal'])
    expect(c('treatment')!.categories).toEqual({ exact: 2 })
    expect(c('covariates')!.levels).toEqual(['interval', 'ratio'])
    expect(spec.constraints.minRule).toEqual({ kind: 'rows-per-group', n: 10 })
  })

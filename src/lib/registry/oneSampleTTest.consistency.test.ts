...
describe('one-sample t-test registry stays faithful to the spec HTML (verbatim, card-scoped)', () => {
  ...
  it('options equal the inputs card option strip; μ₀ number, α adjustable, tails select, CI adjustable', () => {
    ...
    expect(spec.options.map((o) => o.kind)).toEqual(['number', 'number', 'select', 'select'])

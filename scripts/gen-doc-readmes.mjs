...
for (const [nn, id, name, question, fixture, dataCfg, roles, opts] of D) {
  ...
  const isConstruct = id === 'ave' || id === 'composite-reliability'
  const md = `# ${nn} · ${name}
...
${isConstruct ? '**Constructs** (construct-slots input — name each construct, tick its items):' : '**Role assignments** (drag column → slot):'}
...
`
for (const fam of ['Descriptive', 'Group comparison', 'Association', 'Regression', 'Econometrics · time series', 'Econometrics · panel', 'Econometrics · causal', 'Latent variables · reliability', 'Latent variables · factor analysis', 'Data reduction']) {
